...
- **Vector indexes** execute approximate nearest-neighbor search over embeddings, using 1-bit RaBitQ codes with SPFresh-style incremental maintenance and an exact rerank. `search_effort` (0 to 1) trades recall for latency.
...
- **Algebraic indexes** derive aggregate and materialization sidecars from the schema.
- **Relational indexes** are ordered composite-key indexes over relational tables.
...
- `aggregations` compute bucket and metric aggregations over the matching documents.
- `hierarchy` returns direct chunk matches or groups them under their parent documents.
