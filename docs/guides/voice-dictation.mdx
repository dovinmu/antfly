...
The prompt KV cache is on by default in `antfly inference run` and off by default in `antfly standalone`, where `"inference": {"prompt_cache": {"enabled": true}}` turns it on; the same `--config` key works for `antfly inference run` too, and there it can also turn the cache off (`"prompt_cache": {"enabled": false}`). With the cache on, the fixed rule set is served from the prefix cache after the first request with a given dictionary, context, and style, and `usage.cached_prompt_tokens` reports how much of the prompt was reused.
...
- [Index Recorded Calls and Podcasts](/docs/guides/transcription) to transcribe recordings at write time and search them with timestamps.
