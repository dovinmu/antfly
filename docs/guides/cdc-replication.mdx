...
To fan one PostgreSQL table out to several Antfly tables, use `routes` instead. Each route has its own `where` filter, `target_table`, `key_template`, and `on_update`/`on_delete` transforms. Every route whose filter matches a row receives it, and when routes are present the top-level `on_update`/`on_delete` are ignored.
...
The cost of that arrangement is the replication slot. A slot makes PostgreSQL retain WAL until the subscriber has consumed it. While Antfly is up, that is a few seconds of WAL. While Antfly is down for a long stretch, PostgreSQL keeps every change since the last checkpoint, and disk usage grows until the stream resumes. Monitor slot lag the way you would for any logical subscriber (Antfly also reports `status.lag_records` and `status.lag_millis` in the table detail), and if you decommission an Antfly table, drop its slot in PostgreSQL rather than leaving it to hold WAL forever.
