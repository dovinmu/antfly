...
- `generator` (object, required): LLM configuration — `{ provider: "antfly" | "ollama" | "openai" | "openrouter" | "gemini" | "vertex", model, api_key? }`
...
function MyComponent() {
  ...
  const handleAsk = () => {
    startStream({
      ...
      headers: { Authorization: 'ApiKey <encoded>' } // base64(key_id:key_secret)
    });
  };
