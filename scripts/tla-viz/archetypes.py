...
class Binding:
    ...
    def category_legend(self) -> list[dict]:
        cats = [{"label": c["label"], "color": CATEGORY_COLORS[i],
                 "shape": CATEGORY_SHAPES[i]}
                for i, c in enumerate(self.categories[:3])]
        cats += [{"label": "Other", "color": CATEGORY_COLORS[OTHER_INDEX],
                  "shape": CATEGORY_SHAPES[OTHER_INDEX]},
                 {"label": "Faults", "color": CATEGORY_COLORS[FAULT_INDEX],
                  "shape": CATEGORY_SHAPES[FAULT_INDEX]}]
        return cats
...
def load_bindings(path: Path) -> dict[str, Binding]:
    ...
    return {tag: _parse(tag, entry) for tag, entry in raw.items()
