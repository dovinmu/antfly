...
|---|---|---:|---|---|---|---|---|
| Local transaction manager | `AntflyTransaction.tla`, `MC.tla`, `AntflyTransaction.cfgs`, `AntflyTransactionBadSkipIntentConflict` | Checked | `pkg/antfly/src/storage/transactions.zig` `TxnManager`; local transaction callsites in `pkg/antfly/src/storage/db/db.zig` | `transaction protocol fences begin prepare snapshot and idempotent resolution`, `recoverTransactions auto-aborts stale pending transactions`, finalized-orphan/participant cleanup tests, DB OCC predicate and concurrent-intent tests; accepted transaction trace lifecycle | local OCC snapshots and pending-intent exclusion, intent resolution, stale-pending recovery, atomicity, no orphaned intents, LWW/read consistency, cleanup after prepared participants resolve | intentionally excludes the API distributed coordinator, topology fencing, stable terminal responses, and cross-group recovery, which are covered by the focused sibling model | keep local TxnManager correspondence separate from distributed recovery outcomes |
| Distributed transaction recovery | `AntflyDistributedTransactionRecovery.tla`, `.cfgs`, and seven pinned negative checks | Bounded | `pkg/antfly/src/api/distributed_txn.zig`, `pkg/antfly/src/api/transactions.zig`, `pkg/antfly/src/api/table_writes.zig`, `pkg/antfly/src/storage/transactions.zig` | stable-decision retry, topology-change identity, never-abort-after-commit, coordinator recovery, prepared-follower recovery, terminal handoff, and HTTP stable-retry tests | durable decision never reverses; commit requires durable prepare history and the attempt topology epoch; prepared followers do not presume abort; stable retry preserves transaction identity/transforms; cleanup waits for propagation acknowledgement; exact pending/success outcomes reflect recovery and visibility debt | bounded to two participants and two topology epochs; abstracts payload bytes, routing-key selection, transport scheduling, and latency | preserve one negative section per named recovery invariant as current coordinator behavior evolves |
| Transaction traces | `TraceAntflyTransaction.tla`, `TraceAntflyTransaction.cfgs` | Trace-linked | local transaction trace emitter plus the compatibility filter for the accepted `TxnManager` OCC lifecycle | checked transaction trace fixtures and pre-existing live transaction generation | trace consumption plus local transaction safety invariants | recovery-only API outcomes are excluded rather than claimed by this trace model; coverage depends on emitted compatible transactions | use `AntflyDistributedTransactionRecovery` and Zig correspondence tests for excluded recovery outcomes |
| Session savepoints and terminal handoff | `AntflyTransactionSession.tla`, `.cfgs`, `AntflyTransactionSessionBadRollback`, `AntflyTransactionSessionBadRecoveryDecision`, `AntflyTransactionSessionBadCleanup` | Bounded | `pkg/antfly/src/api/transactions.zig` durable `SessionRegistry`/`TerminalCommit`; `pkg/antfly/src/api/http_server.zig` stable retry and cleanup; `pkg/antfly/src/storage/transactions.zig` recovery | durable terminal coordinator handoff, sealed commit retry, HTTP stable terminal retry without write replay, savepoint, stale-pending, finalized-orphan, participant cleanup, and identity-row recovery tests | committed-base visibility is separate from staged writes; rollback discards later intents; finalized recovery follows the durable decision; stable retry preserves transaction ID, coordinator group/table, write and transform application counts; terminal outcomes are exactly committed/recovery-pending/visibility-pending; cleanup retains propagation debt | abstracts concrete keys, versions, payload bytes, leases/principals, transport errors, and multi-session concurrency; local OCC details remain in `AntflyTransaction` | keep session fixtures synchronized with the durable terminal record fields and response mapping |
| Transaction session trace fixtures | `TraceAntflyTransactionSession.tla`, `.cfgs`, `specs/tla/traces/txn_session_*.ndjson`, `specs/tla/traces/negative/txn_session_bad_cleanup.ndjson` | Trace-linked | `pkg/antfly/src/api/transactions.zig`, `pkg/antfly/src/api/http_server.zig`, `pkg/antfly/src/storage/transactions.zig` | checked-in savepoint, finalized-orphan, stale-pending, stable-retry, visibility, and premature-cleanup scenarios | post-action refinement includes stable transaction ID, coordinator group/table, exact terminal outcome, write/transform counts, propagation/visibility debt, terminal retention, and cleanup gating | fixtures are checked in rather than live-emitted; counts abstract concrete writes and transforms; no OCC trace bridge | retain the pinned cleanup negative and require all positive fixtures to assert terminal handoff coordinates and outcomes |
...
| Split terminal topology publication | `AntflyTerminalTopologyPublication.tla`, `.cfgs`, `AntflyTerminalTopologyPublicationBadDropFinalizedIntent` | Bounded | `metadata/reconciler.zig` active-transition range fences, `metadata/state.zig` projected-state seeding, `metadata/table_manager.zig` terminal outcome folding | finalized split/merge seeding regressions and automatic-split multi-node E2E | a finalized intent remains represented until its range topology is folded; terminal compaction requires the two-range topology after cutover | one split, one-to-two range publication, no Raft apply failure or concurrent table mutation; no live runtime trace family yet | emit catalog-transition/range-publication events and add a trace-refinement validator for the two-reconcile handoff |
...
| Raft Ready pipeline | `AntflyRaftReadyPipeline.tla`, `.cfgs`, scheduling, ownership, message, and membership Bad* configs | Checked | `lib/raft/src/runtime/scheduler.zig`; `lib/raft/src/runtime/multi_raft.zig` `MultiRaft.processReadyIntoOutbox`, message cloning, `applyCommittedConfChanges`, and applied-index snapshot candidates | `runtime_test.zig` tests `multi raft backpressure rejects async ready before cloning messages`, `drainReady continues async pipeline without starving peer`, `does not retry a no-progress frontier`, `reserves continuations for productive groups`, and `apply queue drains with per-round budget`; `multi_raft.zig` test `snapshot candidate keeps configuration at its applied index` | fair coverage precedes productive continuations; denied work owns no messages; Ready messages are cloned/owned before committed membership mutates Raft state; messages survive apply; snapshot membership matches its applied index; budget remains bounded | abstracts payload bytes, persistence/transport failure after ownership, quiesce/register churn, and wall-clock diagnostics | retain the fast projections and composed heavy liveness check |
...
| LMDB commit/readers | `AntflyLmdbCommit.tla`, `.cfgs`, `AntflyLmdbCommitBadMetaBeforeData`, `AntflyLmdbCommitBadReaderReuse` | Bounded | `pkg/antfly/src/lmdb/txn.zig`; `prepare_commit_support.zig` `CommitPublishPhase`/`PreparedCommit`; `commit_support.zig` `publishPreparedCommit`/`publishPreparedCommitSync`; `free_db.zig`; `readers.zig` | transaction reopen tests before data sync, after data sync/before meta publish, and after meta write/before meta sync; active-reader delayed reuse and oldest-reader reclaim tests; nested child transaction tests | two meta-page selection, prepared data pages, data-sync before meta-write, crash/reopen meta choice, reader snapshots over page images, free-record reuse gated by oldest reader, child commit/abort merge shape | abstracts exact B-tree/page/checksum/overflow bytes, named DB metadata, local reader mmap remap lifetime, `sync`/`worker_thread`/`async_io` scheduling, and `no_sync`/`no_meta_sync`/`write_map`/`map_async`/`fixed_map` policy variants | retain crash-phase and reader-reuse tests; no model ordering change is required for the current commit-support refactor |
...
| Derived replay | `AntflyDerivedReplay.tla`, `.cfgs`, heavy and expected-failure configs; `TraceAntflyDerivedReplay.tla`, `.cfgs`, positive and expected-failure fixtures | Trace-linked | `pkg/antfly/src/storage/db/derived/replay_source.zig`, `derived_worker.zig`, `derived_replay_trace.zig`, primary-store replay rows/metadata in `docstore.zig`, dense bulk replay paths in `db.zig` | thin replay, replay catch-up, reopen, truncation, cutover/reopen, and replay-source primary-store tests emit live hint-lane and replay-all fallback scans under `-Dwith_tla=true` | applied watermarks cannot skip targeted replay; hint-lane accounting is honest; an empty or unavailable hint lane falls back to replay-all; query targets stay within applied/target; truncation keeps needed replay; bulk sessions block target/query advancement | scan budgets and payload decoding remain abstract; a missing earlier hint row followed by a later visible hint row is still a publication-integrity obligation because fallback begins after the visible row | keep live fallback traces in smokeouts and add an atomic replay-row publication check if the storage writer is made interruptible |
| Generated enrichment runtime | `AntflyEnrichmentLease.tla`, `.cfgs`, stale-publish, empty-pending, and terminal-worker-poison mutants; `TraceAntflyEnrichmentLease.tla`, `.cfgs`, positive and expected-failure fixtures | Trace-linked | `pkg/antfly/src/storage/db/enrichment/enrichment_runtime.zig` `foregroundCatchUpDecision`, `retryBudgetAllowsYield`, `recordIsolatedRequestError`, `flushGeneratedReplayWindow`, `runForegroundCatchUpPassOwned`; `enrichment_state.zig` persisted runtime status; generated artifact/replay paths in `db.zig` | tests `enrichment foreground catch up reconciles retry state after checkpoint apply`, `enrichment worker attempt budget includes the current request`, `enrichment worker retry delay is exponential and capped`, `enrichment runtime restore preserves retry target across restart`, `terminal coverage revalidates durable debt under the failure fence`, `asset batch fallback keeps the logical request retry budget`, and `persisted lease validity detects takeover before cached ownership does` | target/applied ordering; replay visibility and current persisted lease precede generated publication/checkpoint progress; hidden generated work cannot be skipped; terminal request failures publish index-scoped repair debt/coverage without poisoning the worker | provider payloads, exact embedding/chunk/artifact bytes, retry durations, and window byte/item limits are intentionally routed to deterministic Zig tests; split-range composition remains outside this bounded module | retain the focused retry-budget, terminal-isolation, failure-fence, applied-watermark, and persisted-lease takeover tests |
| Replay/enrichment restart bridge | `AntflyReplayEnrichmentBridge.tla`, `.cfgs`, retention, false-advance, restart-rearm, retry-boundary, and dropped-repair-debt mutants | Bounded | `pkg/antfly/src/storage/db/enrichment/enrichment_runtime.zig` persisted `RuntimeStatus`, supervised retry budget, terminal coverage handoff, and startup worker loop; async replay truncation/applied checkpoints in `pkg/antfly/src/storage/db/db.zig` | generated-enrichment replay-retention regression; 429→restart E2E; runtime restore, attempt-budget, capped-delay, durable failure-fence, and terminal-isolation tests | replay remains retained through every durable consumer watermark; retries cross scheduler boundaries and remain bounded; exhaustion records durable repair debt before terminal coverage; restart preserves retry/debt state and rearms unfinished work; applied cannot pass unresolved coverage debt | source/provider payloads, exact backoff duration, artifact bytes, process identity, and scan batch shape are routed to deterministic Zig tests instead of larger TLC bounds | retain the 429→restart, retry-exhaustion, repair-ledger-fence, and truncation/applied-watermark gates |
| Replay delete supersession | `AntflyReplayDeleteSupersession.tla`, `.cfgs`, retry-deleted mutant | Bounded | thin replay source reads, document-identity tombstones, and full-text/dense-field/sparse-field apply paths in `storage/db/db.zig` | deterministic TTL-delete-before-derived-replay regression and TTL/transaction bucket | a missing replay source advances only when durable identity proves that a later delete superseded it; unknown missing sources remain retryable; durable deletes eventually drain replay and index visibility | one document and one abstract derived index; exact journal encoding, batching, range handoff, and retry timing remain concrete-test concerns | retain a mixed write/delete/TTL process soak and add trace events only if missing-source classification changes |
| Live source cardinality after TTL | `AntflyLiveSourceCardinality.tla`, `.cfgs`, pre-move-owner and live-cardinality-preservation mutants | Bounded | TTL batch context, document-identity visibility cache, managed-writer owner binding, and runtime snapshot anti-regression in `storage/db/db.zig` / `api/runtime_status.zig` | TTL callback cache regression, runtime-status cache regression, and blocked-provider TTL production E2E | a TTL delete refreshes the stable managed DB owner; live source-count decreases are authoritative even while derived artifact facts are preserved; a durable delete eventually publishes zero source coverage | one DB move, one source-count bit, and one abstract status publication; exact TTL scan timing, batches, payloads, and index artifact counters remain concrete-test concerns | retain the blocked-provider TTL server regression; add a trace family only if DB wrapper ownership or runtime-status publication changes again |
| Enrichment lifecycle lock order | `AntflyEnrichmentLifecycleLockOrder.tla`, `.cfgs`, blocking-status mutant | Bounded | enrichment lifecycle supervision, structural index mutation, enrichment publication, and runtime status in `storage/db/db.zig` | lifecycle-mutex nonblocking status regression, rate-limited managed delete unit test, and repeated fresh-process delete/recreate E2E | status holding the DB apply lock cannot wait for enrichment lifecycle ownership; a lifecycle stop/join converges after a worker publication is admitted | starts at the captured three-party adverse interleaving and collapses worker publication and runtime stop internals; does not model reconfigure pointer replacement or mutex implementation details | retain a repeated rate-limited delete/recreate workload and add implementation lock events only if lifecycle ownership grows another participant |
| Enrichment lease denial backoff | `AntflyLeaseRetryBackoff.tla`, `.cfgs`, immediate-retry mutant; denial/retry events in `TraceAntflyEnrichmentLease` | Trace-linked | lease-denial path in `enrichment_runtime.zig` | shared-store 50 ms retry-rate regression plus emitted denial/retry-ready events | every denied acquisition has a wait boundary before another attempt; acquisition follows durable lease release | TLC does not measure the 25 ms duration or CPU use | keep the retry-rate watchdog in the runtime gate |
| Public table admission | `AntflyTableAdmission.tla`, `.cfgs`, persist-invalid mutant | Bounded | public table/index parsing boundary in `api/table_contract.zig` | parser matrix and create/retry E2E | rejected unsupported index kinds leave no desired or committed catalog residue and the name remains reusable | metadata transport and reconciliation are abstracted after the admission boundary | preserve the unsupported-create then valid-retry gate |
...
| Placement readiness aggregation | `AntflyPlacementReadiness.tla`, `.cfgs`, B1 unknown-report ambiguity and leader-only admission Bad* configs; `TraceAntflyPlacementReadiness.tla`, `.cfgs`, positive and expected-failure fixtures | Trace-linked | metadata state/reconciler group-status aggregation, stable-placement predicate, hosted shard transition admission, `metadata/placement_trace.zig` | metadata logic tests emit qualified/unknown/conflicting report batches and transition admission facts under `-Dwith_tla=true`; extracted live traces plus positive/negative fixtures replay through TLC | unknown voter-set reports may seed fallback counts but cannot erase authoritative qualified voter-set evidence; conflicting fingerprints fail closed without leader authority; transition admission requires exact known healthy non-joint placement | trace facts abstract voter identities to a fingerprint and do not model reporter transport loss before metadata receives the report | run the smokeout autosplit/mixed workloads with trace extraction and retain failing live segments as fixtures if a new mechanism appears |
| Runtime status and schema reconciliation | `AntflyRuntimeStatusReconciliation.tla`, `.cfgs`, precedence, disk provenance, join, migration, and read-availability Bad* configs | Bounded | runtime-status preference/overlay; `pkg/antfly/src/data/runtime.zig` root-generation publication; metadata schema finalization; HA read gate | tests `idle cached runtime status stays fresh only for published root generation`, `runtime status disk usage cache is scoped to one root generation`, `runtime status disk scan retries only when maintenance invalidates its group`, and `data runtime stamps one producer generation on every reported group`; complete fresh disk/join, schema E2E, and standby read tests | old/removed/wrong-root observations cannot win; known disk facts match their root; join input is complete/fresh; finalization requires reported current coverage; read schema stays available | exact disk scans/cache invalidation are deterministic-test concerns; `db.zig` `indexRepairScanDue` is a separate IndexLifecycle scheduler contract, not runtime-status ordering | retain the current root-generation/evidence guards; compose schema progress with scheduler-aware `AntflyIndexLifecycle` |
| Index rebuild lifecycle | `AntflyIndexLifecycle.tla`, `.cfgs`, incomplete-swap, stale-recovery, lost-immediate-plus-fallback-wakeup, and accepted-write-loss Bad* configs; `AntflyRepairCoverageRetry.tla`, `.cfgs`, stale-counter and stale-repair-fence mutants; `AntflyRepairArtifactRecovery.tla`, `.cfgs`, missing-reprocess, undrained-recovery, stale-candidate, stale-startup-counter, and omitted-startup-rearm mutants; `TraceAntflyIndexLifecycle.tla`, `.cfgs`, positive and expected-failure fixtures | Trace-linked | `pkg/antfly/src/data/runtime.zig` `indexRepairScanDue` and `indexRepairSchedulerBackoffBlocks`; `pkg/antfly/src/storage/db/db.zig` durable repair intent revisions/pins, bounded foreground artifact recovery, startup reconciliation, and shadow activation; `derived/index_repair_state.zig` fenced transitions; `enrichment/enrichment_runtime.zig` terminal coverage failure fence; catalog/index-manager generation coverage; `storage/db/index_lifecycle_trace.zig` | tests `index repair scan periodically rediscovers...`, `fallback backoff never blocks...`, `lost-wakeup fallback stays bounded...`, `fallback advances past a non-local prefix...`, `index repair treats source coverage lag as retryable`, `db dense repair reprocesses corrupt managed source before rebuilding`, `db restart automatically resumes pinned explicit dense generation rebuild`, `db restart reconciles activated dense repair without rebuilding`, and `terminal coverage revalidates durable debt under the failure fence`; checked lifecycle fixtures | every accepted post-admission write reaches snapshot or replay debt before generation publication; each committed generation has durable work; exact dirty wakes bypass fallback cadence/backoff, while bounded rediscovery rearms lost wakes; intent revision fences coverage/activation; incomplete coverage cannot activate; current-generation outcomes supersede stale name counters; managed source repair debt survives restart, rearms the bounded foreground owner, drains, discards the failed candidate, and rebuilds before stable activation | fairness remains explicit; provider payloads, corpus/artifact bytes, exact retry delays, scan batch sizes, and candidate bytes are routed to deterministic Zig tests; trace replay proves observed ordering rather than unconditional scheduling | retain quiet/mixed-load multi-generation, lost-wakeup fallback, accepted-write publication, repair-fence, corrupt-artifact foreground, and restart-resume gates |
...
| ML graph passes/runtime | `AntflyMlGraphPasses.tla`, `.cfgs`, `AntflyMlGraphPassesBadDanglingCse`, `AntflyMlGraphPassesBadParameterDedup`, `AntflyMlGraphPassesBadMissingLowerClosure`, `AntflyMlGraphPassesBadFallbackRuntime`, `AntflyMlGraphPassesBadPartialPublish` | Bounded | `lib/ml/src/graph/passes/{pipeline,cse,dce,fuse}.zig`, `lib/ml/src/graph/lower.zig`, `pkg/inference/src/graph/{partition_export,runtime,pjrt_compiler}.zig` | focused graph-pass/lower/export/runtime tests plus PJRT tests `PJRT compiler can externalize graph parameters as inputs`, `semantic KV option adds past inputs and present outputs`, `semantic KV output selection keeps only final graph output`, and `semantic decode inputs cover normal cache-writing attention` | pass output ordering, valid current/exported references, output preservation, parameter/constant identity, fused primitive lower closure, external runtime-input materialization, failed-output isolation, and fail-closed fallback publication | bounded representative graph; abstracts concrete op/hash/shape equality, fixed-point counts, exact GQA/RoPE matrices/layout/HLO bytes, backend capabilities, ONNX lowering, scheduling, and executor lifetimes; concrete GQA/RoPE lowering stays in PJRT tests | retain focused pass/export/runtime tests and keep compiler output selection/externalization in the compiler-specific model |
...
| ML compiler publication | `AntflyMlCompilerPublication.tla`, `.cfgs`, `AntflyMlCompilerPublicationBadStaleCompile`, `AntflyMlCompilerPublicationBadMissingInput`, `AntflyMlCompilerPublicationBadOutputSelection`, `AntflyMlCompilerPublicationBadFallbackPublish`, `AntflyMlCompilerPublicationBadPartialArtifact` | Bounded | `pkg/inference/src/graph/partition_export.zig`, `pkg/inference/src/graph/pjrt_compiler.zig`, `pkg/inference/src/graph/runtime.zig` | PJRT tests `PJRT compiler can externalize graph parameters as inputs`, `semantic KV option adds past inputs and present outputs`, `semantic KV output selection keeps only final graph output`, and `semantic decode inputs cover normal cache-writing attention`; partition-export and runtime gate/native-executor tests | partition export materializes parameters and semantic KV inputs, includes compiler outputs, and yields complete artifacts; runtime publishes only fresh graph/export versions, hides failed artifacts, selects exact final outputs, and fails closed on fallback partitions | exact HLO/native artifact bytes, GQA/RoPE matrices and layouts, dynamic shapes, planner search, backend capability matrices, cache-key details, compile concurrency, and executor lifetime are deterministic compiler/runtime concerns; current PJRT changes do not alter modeled publication ordering | retain exact input/output-selection tests and add artifact hash/refinement only if compiler outputs become traceable |
...
| Bounded | `AntflyPlacementReadiness` and `AntflyIndexLifecycle` (+live trace validators; bounded liveness models remain explicit abstractions), `AntflyShardSplit` and `AntflySplitRefinementBridge` (+trace fixtures; stable-placement bridge newly added), `AntflyRuntimeStatusReconciliation`, `AntflyHAFailoverSafety`, `AntflyHAPartitionFence`, `AntflyHAGateTransitions`, `AntflyHAGates`, `AntflyCdcCutover` (anchor correction pending), `AntflyQueryCompleteness`, `AntflyShardSplitSeq`, `AntflySnapshotContent`, `AntflyLsmReserveCleanup`, `AntflyDbSplitVisibility`, `AntflyLsmLifecycle`, `AntflyNodeDrainLifecycle`, `AntflyTableLifecycle`, `AntflyHARetentionReseed` |
...
| `AntflyDistributedTransactionRecovery` | commit decision recorded without every participant having prepared | `CommitRequiresAllPrepared` |
| `AntflyDistributedTransactionRecovery` | commit decision recorded under a stale topology epoch | `CommitUsesCurrentTopology` |
| `AntflyDistributedTransactionRecovery` | durable commit reversed to abort | `DecisionNeverReverses` |
| `AntflyDistributedTransactionRecovery` | prepared follower presumes abort without coordinator authority | `PreparedFollowerDoesNotPresumeAbort` |
| `AntflyDistributedTransactionRecovery` | stable retry creates a new transaction and reapplies transforms | `StableRetryPreservesIdentityAndWrites` |
| `AntflyDistributedTransactionRecovery` | coordinator cleanup drops pending propagation debt | `CleanupRequiresRecoveryComplete` |
| `AntflyDistributedTransactionRecovery` | success reported while propagation or visibility debt remains | `SuccessReflectsPropagationAndVisibility` |
...
| `AntflyReplayEnrichmentBridge` | fast consumer truncates replay past enrichment, restart loses volatile work, and an empty scan falsely advances | `RetainedUntilAllConsumersApplied` / `NoAdvancePastCoverageDebt` |
| `AntflyReplayDeleteSupersession` | a durably tombstoned replay source is retried forever ahead of its later delete | `DurableDeleteEventuallyConverges` |
| `AntflyLiveSourceCardinality` | TTL updates a pre-move DB owner, or artifact anti-regression overwrites a live zero source count | `DeleteEventuallyPublishesZero` |
| `AntflyLeaseRetryBackoff` | replacement worker retries a denied durable lease without a wait boundary | `DeniedAttemptsHaveWaitBoundaries` |
| `AntflyTableAdmission` | unsupported index request persists desired/committed metadata before returning rejection | `RejectedLeavesNoResidue` |
