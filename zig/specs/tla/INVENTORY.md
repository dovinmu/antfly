...
| Index rebuild lifecycle | `AntflyIndexLifecycle.tla`, `.cfgs`, incomplete swap, stale recovery, and lost-second-wakeup Bad* configs; `TraceAntflyIndexLifecycle.tla`, `.cfgs`, positive and expected-failure fixtures | Trace-linked | DB catalog/index manager, durable repair intent/status snapshots, managed admission outbox, worker capacity admission, shadow pointer activation, `storage/db/index_lifecycle_trace.zig` | focused managed full-text admission/restart and second-wakeup tests emit live lifecycles under `-Dwith_tla=true`; extracted traces plus two-generation positive and lost-wakeup negative fixtures replay through TLC | each committed managed generation publishes durable work; worker admission follows queued work; intent phases remain tied to the config hash; replacement activation and ready retirement are observed | liveness still depends on the main model's explicit scheduler fairness; trace replay proves observed ordering, not that an indefinitely idle process must eventually schedule | run quiet and mixed-load multi-generation smokeouts; retain any deferred/failure segment and refine the model if the actual stall mechanism differs |
