...
| Generated enrichment runtime | `AntflyEnrichmentLease.tla`, `.cfgs`, stale-publish and empty-pending mutants; `TraceAntflyEnrichmentLease.tla`, `.cfgs`, positive and expected-failure fixtures | Trace-linked | `pkg/antfly/src/storage/db/enrichment/enrichment_runtime.zig`, `enrichment_worker.zig`, `enrichment_trace.zig`, generated artifact publication/replay paths in `db.zig` | generated dense/sparse/chunk, retry, restore, split/merge cutover, and deterministic lease-takeover tests emit live durable-lease observations under `-Dwith_tla=true` | target/applied ordering; replay visibility before generated publication; publication and applied-checkpoint advancement require the durable lease to still name the owner and remain unexpired; hidden generated work cannot be skipped; isolated failures do not poison the worker | trace checking observes the durable lease at event time but does not force provider calls to exceed the lease TTL; exact payload bytes, provider timing, backoff, and split-range composition remain abstract | add a slow-provider/two-owner runtime test; revalidate or renew the durable lease before publishing generated artifacts and advancing the applied checkpoint |
| Replay/enrichment restart bridge | `AntflyReplayEnrichmentBridge.tla`, `.cfgs`, retention and false-advance mutants | Bounded | async replay truncation and generated-enrichment durable applied checkpoint in `storage/db/db.zig` | focused generated-enrichment retention regression and 429→restart E2E | replay remains retained through every durable consumer watermark; restart cannot advance enrichment past unreconstructible coverage debt; debt drains after provider recovery | source documents and payload bytes are abstracted to sequence debt; process identity and exact scan batches remain trace/harness concerns | retain the 429→restart gate and correlate truncation with coverage status in smokeouts |
...
| Index rebuild lifecycle | `AntflyIndexLifecycle.tla`, `.cfgs`, incomplete swap, stale recovery, and lost-second-wakeup Bad* configs; `AntflyRepairCoverageRetry.tla`, `.cfgs`; `AntflyRepairArtifactRecovery.tla`, `.cfgs`, missing-reprocess, undrained-recovery, stale-candidate, and stale-startup-counter mutants; `TraceAntflyIndexLifecycle.tla`, `.cfgs`, positive and expected-failure fixtures | Trace-linked | DB catalog/index manager, durable repair intent/status snapshots, managed admission outbox, worker capacity admission, generation-scoped coverage outcomes, managed artifact reprocessing, bounded foreground recovery, startup rebuild planning, shadow pointer activation, `storage/db/index_lifecycle_trace.zig` | focused managed full-text admission/restart, retryable source-coverage classification, current-incarnation target selection, corrupt-artifact foreground reprocessing, post-repair startup-plan stability, and second-wakeup tests emit live lifecycles under `-Dwith_tla=true`; extracted traces plus two-generation positive and lost-wakeup negative fixtures replay through TLC | each committed managed generation publishes durable work; worker admission follows queued work; intent phases remain tied to the config hash; incomplete source coverage cannot activate; complete current-generation outcomes supersede a stale name-scoped artifact counter in both shadow activation and later startup planning; a coverage-failed managed candidate reprocesses and drains its source artifacts in the bounded repair owner, then is discarded before rebuilding; replacement activation and ready retirement are observed | liveness still depends on the models' explicit scheduler fairness; artifact recovery collapses bounded provider retry and the corpus scan to one foreground completion action; trace replay proves observed ordering, not that an indefinitely idle process must eventually schedule | run quiet and mixed-load multi-generation smokeouts; retain any deferred/failure segment and refine the model if the actual stall mechanism differs |
