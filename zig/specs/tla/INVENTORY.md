...
|---|---|---:|---|---|---|---|---|
| Distributed transactions | `AntflyTransaction.tla`, `MC.tla`, `AntflyTransaction.cfgs`, `AntflyTransactionBadSkipIntentConflict` | Mature | `pkg/antfly/src/storage/db/db.zig`, transaction recovery/runtime code | transaction lifecycle, OCC predicate, participant recovery tests; transaction trace validation | atomicity, no orphaned intents, OCC serialization, LWW/read consistency | session savepoints and identity side effects are mostly outside this model; only one core mutant is currently wired | add a second negative config for orphaned-intent cleanup if per-invariant mutant coverage is required |
| Transaction traces | `TraceAntflyTransaction.tla`, `TraceAntflyTransaction.cfgs` | Trace | transaction trace emitter/filter scripts | `make tla-trace-txn TRACE_FILES=...` | trace consumption plus transaction safety invariants | trace filtering can hide unsupported events; coverage depends on emitted test traces | add documented trace generation command and malformed-trace negative check |
...
| Managed host lifecycle | `AntflyManagedHostLifecycle.tla`, `.cfgs`, lifecycle plus mismatched-artifact and catalog-binding Bad* configs | Mature | managed raft host/catalog/bootstrap code and reconciler restore projection | managed-host restore, restart, removal, and catalog tests | activation requires verified exact artifact identity; restored catalog retains that binding; routes/active/catalog remain desired and durable across failure, cancellation, and restart | abstracts artifact bytes, peer lists, WAL/proposal persistence, leader election, and bootstrap error taxonomy | add host simulation traces only if catalog/route ordering changes |
| Placement membership repair | `AntflyPlacementRepair.tla`, `.cfgs`, repair, evidence, contraction, and liveness Bad* configs | Mature | placement planner repair and reconciler membership-transition index | focused duplicate recovery, stable retry, final-membership latch, per-store proof, and source-retirement tests | quota-full duplicate recovery retains the advanced member; replacement identity is stable; final membership is latched; only exact live-leader evidence from the authoritative store permits retirement; stale followers cannot block contraction | one group/replacement; abstracts scoring, bytes, and Raft joint-consensus internals | retain fast safety projections and heavy contraction liveness |
| Runtime status and schema reconciliation | `AntflyRuntimeStatusReconciliation.tla`, `.cfgs`, precedence, disk provenance, join, migration, and read-availability Bad* configs | Mature | runtime-status preference/overlay, data runtime root generation, metadata schema finalization, HA read gate | runtime-status authority, complete fresh disk/join, schema E2E, and standby read tests | old/removed/wrong-root observations cannot win; known disk facts match their root; join input is complete/fresh; every hosted range/node reports current schema progress before finalize; read schema stays available | abstracts JSON, exact byte totals, schema semantics, index internals, and network transport | keep one status-envelope protocol with fast safety and heavy migration liveness |
...
| Tier | Models |
|---|---|
| Mature | `AntflyBatcherCoalescing` (red->green correspondence test), `AntflyDerivedReplay` (executed co-write tests), `AntflyEnrichmentLease`, `AntflyLmdbCommit`, `AntflyLsmWalCompaction`, `AntflyTransaction`, `AntflyTransactionSession` (+trace fixtures), `AntflyShardSplit`, `AntflySnapshotTransfer`, `AntflyRaftSchedulerFairness`, `AntflyRaftReadyPipeline`, `AntflyRuntimeStatusReconciliation`, `AntflyPlacementRepair`, `AntflyDocumentIdentity`, `AntflyDocumentIdentityRangeRepair` (+trace fixtures), `AntflyHASyncWait`, `AntflyHATimelineSwitch`, `AntflyHAStandbyApply`, `AntflyHARejoin`, `AntflyHAReplication`, `AntflySplitRefinementBridge` (+trace fixtures), `AntflyLitePublication`, `AntflyMlGraphPasses`, `AntflyMlGraphDagPasses`, `AntflyMlCompilerPublication`, `AntflyManagedHostLifecycle` |
