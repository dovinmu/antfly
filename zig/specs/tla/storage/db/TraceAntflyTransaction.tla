...
\* MaxTimestamp: bound on the abstract model clock.
\* The spec clock is NOT the real nanosecond timestamp — it just counts
\* actions. Len(TraceLog) * 2 provides room for trace steps + TickClock.
TraceMaxTimestamp == Len(TraceLog) * 2 + 2
...
ResolveDecisionConflictIfLogged(t) ==
    /\ LoglineIsTxnEvent("ResolveDecisionConflict", t)
    /\ UNCHANGED vars
    /\ StepToNextTrace
...
\* TickClock is disabled during trace validation: the abstract model clock
\* only exists for state-space bounding during model checking, not for
\* replaying concrete traces. InitTransaction increments clock, which is
\* sufficient to satisfy the clock < MaxTimestamp guard.
TickClockIfNeeded == FALSE
