...
pub fn expectPublicIndexRuntimeStatusMetadata() !void {
    ...
    try std.testing.expect(@hasField(indexes_generated.FullTextIndexStats, "projection_checkpoint_config_fingerprint"));
    ...
    try std.testing.expect(@hasField(indexes_generated.EmbeddingsIndexStats, "projection_checkpoint_config_fingerprint"));
    ...
    try std.testing.expect(@hasField(indexes_generated.AlgebraicIndexStats, "projection_checkpoint_config_fingerprint"));
...
test "generated route policy inventory is unique and describes wire modes" {
    ...
    for (server.routes, 0..) |route, index| {
        ...
        if (std.mem.eql(u8, route.operation_id, "researchAgent"))
            found_streaming_research = route.request_body == .buffered and route.streaming_response;
    ...
    try std.testing.expectEqual(.query, request_admission_policy.publicOperationClass("researchAgent").?);
    try std.testing.expectEqual(.query, request_admission_policy.publicOperationClass("advanceResearchJob").?);
